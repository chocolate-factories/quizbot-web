...
const InputField: React.FC<{ onSubmit: (arg0: string) => void }> = ({
    onSubmit
}) => {
...
const Quiz: React.FC<QuizParams> = ({
    ...
    const [words, setWords] = useState(() =>
        generateWordList(numberOfWords, categories)
    )
    ...
    const [replaceSpecialCharacters, setReplaceSpecialCharacters] = useState(
        false
    )
        ...
        <>
            {!gameOver ? (
                <Box>
                    ...
                    {inputState ===
                        TranslationInputState.validation_correct && (
                        <Box>
                            <Button
                                color="primary"
                                variant="contained"
